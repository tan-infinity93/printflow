...
//   // SVG version of "edit mode" usually involves a foreignObject or overlay
//   if (elementType == "textBox") {
...
//     // Check SVG classes (use .baseVal for compatibility if needed)
...
//     // SVG elements don't have style.left, so we pull from attributes or dataset
//     // Fallback to the current 'x' attribute if dataset is empty
//     startLeftMm =
//       parseFloat(box.dataset.leftMm) || parseFloat(box.getAttribute("x")) || 0;
//     startTopMm =
//       parseFloat(box.dataset.topMm) || parseFloat(box.getAttribute("y")) || 0;
...
//     // 2. Direct SVG Attribute Update (Visual Move)
//     // We update 'x' and 'y' directly for text/rect
//     // box.setAttribute("x", updatedX);
//     // box.setAttribute("y", updatedY);

//     box.setAttribute("transform", `translate(${updatedX}, ${updatedY})`);

//     const transform = box.getAttribute("transform");
//     const match = /translate\(([^,]+),\s*([^)]+)\)/.exec(transform);
//     startLeftMm = match ? parseFloat(match[1]) : 0;
//     startTopMm = match ? parseFloat(match[2]) : 0;

//     // 3. Keep your original update function if it handles other sync logic
...
//     // if (typeof resolveVerticalCollisions === "function") {
...
const setDefaults = (pageSize) => {
  document.getElementById("title-version").innerText = "Version 3";
  document.getElementById("nav-version").innerText =
    "Printing - 2026 (Version 3)";
